  ...
  function changeFlavour(index: number) {
    ...
    tl.to(
      sodaCanRef.current.rotation, {
        y: index > currentFlavourIndex ? `+=${Math.PI * 2 * SPIN_ON_CHANGE}` : `-=${Math.PI * 2 * SPIN_ON_CHANGE}` ,
        ease: "power2.inOut",
        duration: 1
      }, 0
    )
    ...
    .to(
      {},
      {onStart: () => setCurrentFlavourIndex(nextIndex)}, 0.5
    )
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-[auto,auto,auto] items-center">
        {/* Left */}
        <ArrowButton onClick={() => changeFlavour(currentFlavourIndex - 1)} direction="left" label="Previous Flavour"/>
        {/* Can */}
        <View className="aspect-square h-[70vmin] min-h-40">
          <Center position={[0, 0, 1.5]}>
            <FloatingCan flavour={FLAVORS[currentFlavourIndex].flavour} floatIntensity={0.3} rotationIntensity={1} ref={sodaCanRef}/>
          </Center>
          <Environment files="hdr/lobby.hdr" environmentIntensity={0.6} environmentRotation={[0, 3, 0]}/>
          <directionalLight intensity={6} position={[0, 1, 1]} />
        </View>
        {/* Right */}
        <ArrowButton onClick={() => changeFlavour(currentFlavourIndex + 1)} direction="right" label="Next Flavour"/>
      </div>
